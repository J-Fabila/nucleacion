...
def RelaxGeometry(atoms,config,index):
    ...
    i = 0
    while i < config["relaxation_steps"]:
        dyn.step()
        ...
        if i % 100 == 0:
            ...
            if max_dist > 3*rho: #float(config["dist_from_seed"]):
                break
#            connectivity = CheckConnectivity(dyn.atoms, 3.5)
